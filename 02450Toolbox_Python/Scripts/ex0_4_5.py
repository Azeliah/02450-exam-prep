...
x = np.asmatrix(np.arange(1, 11))
y = np.asmatrix(np.arange(10, 0, -1))
...
m2 = np.mat(np.random.rand(3, 3) * 20)
...
print(m1 > m2)  # element-wise comparison
...
print(m3.mean(axis=0))  # mean values of the columns
print(m3.mean(axis=1))  # mean values of the rows
print(m3 - m3.mean(axis=1))
print((m3 - m3.mean(axis=1)).mean(axis=1))
m3.transpose()  # transpose, also: m3.T
print(m2.I)  # compute inverse matrix
print(m2 * m2.I)
